	...
	class wpUpdaterUms {
		...
		static public function getInstance($pluginDir, $pluginFile = '', $pluginSlug = '') {
			...
			if(!isset($instances[ $instKey ])) {
				$instances[ $instKey ] = new wpUpdaterUms($pluginDir, $pluginFile, $pluginSlug);
			}
		...
		public function checkForPluginUpdate($checkedData) {
			...
			if (empty($request_args['version']))
				return $checkedData;
			if(class_exists('frameUms') && frameUms::_()->getModule('license') && !frameUms::_()->getModule('license')->getModel()->isExpired()) {
		...
		public function prepareRequest($action, $args) {
			...
			return array(
				...
				'user-agent' => $this->_getUserAgentHash(). '/' . $wp_version . '; ' . UMS_SITE_URL. ';'. $this->getIP()
			);
		}
